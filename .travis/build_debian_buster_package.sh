...
sudo docker run -v ${PWD}:/source -v ${PWD}/docker-build-release:/build build/debian:buster \
    /bin/bash -c "cmake -DINSTALL_LIBAPPIMAGE=On -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Release /source && make -j`nproc` && cpack -G DEB -G DEB -R 0.1.0-debian-buster"
