...
sudo docker run -v ${PWD}:/source -v ${PWD}/docker-build-release:/build build/ubuntu:bionic \
    /bin/bash -c "cmake -DINSTALL_LIBAPPIMAGE=On -DINSTALL_ATTICA=On -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Release /source && make -j`nproc` && cpack -G DEB -R 0.1.1-ubuntu-bionic"
